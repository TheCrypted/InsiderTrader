import { Link } from 'react-router-dom'
import NavLink from './NavLink'
import SearchBar from './SearchBar'

export default function Header() {
  return (
    <header className="sticky top-0 z-50 bg-black text-white relative">
      <nav className="flex items-stretch justify-between h-20 relative">
        {/* Logo - Clickable link to home */}
        <Link to="/" className="flex items-center pl-16 hover:opacity-80 transition-opacity duration-200 group">
          <div className="flex items-center gap-2">
            {/* White square with black I that turns yellow background on hover */}
            <div className="w-10 h-10 bg-white group-hover:bg-[#FFFF00] flex items-center justify-center flex-shrink-0 transition-colors duration-200">
              <span className="text-black text-xl">I</span>
            </div>
            {/* TRADER text */}
            <span className="text-xl tracking-tight">TRACKER</span>
          </div>
        </Link>

        {/* Browse and Graph Buttons */}
        <div className="flex items-stretch h-full pr-16 gap-0">
          <Link
            to="/browse"
            className="flex items-center px-4 py-2 bg-black text-white font-normal hover:bg-stone-900 transition-all duration-200"
          >
            Browse All
          </Link>
          <Link
            to="/graph"
            className="flex items-center px-4 py-2 bg-black text-white font-normal hover:bg-stone-900 transition-all duration-200"
          >
            Graph
          </Link>
        </div>
      </nav>
    </header>
  )
}

