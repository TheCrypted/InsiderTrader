...
MODEL_PATH  = latest_file("models/bill_prediction_ensemble_*.pkl")
SCALER_PATH = latest_file("models/feature_scaler_*.pkl")
FEATS_PATH  = latest_file("models/feature_names_*.json")
...
# Feature engineering
# =========================
def engineer_20_features_from_bill(bill: Dict[str, Any]) -> Dict[str, Any]:
    """
    Build exactly your 20 numeric features from Congress.gov JSON.
    ✅ Uses introducedDate for days_since_intro.
    ✅ Uses latest action date for stock features.
    """
    ...
    majority_ratio = (dem / len(sponsors)) if sponsors else 0.5
    ...
    is_house_bill = 1 if bill_type == "hr" else 0
    is_senate_bill = 1 if bill_type == "s" else 0

    total_support = len(sponsors) + safe_int(num_cosponsors)
    has_cosponsors = 1 if safe_int(num_cosponsors) > 0 else 0

    policy_area = ((bill.get("policyArea") or {}).get("name")) or ""
    stocks = stock_features(last_action_date or intro_date, policy_area)
    ...
    feats = {
        ...
        "sponsor_majority_party": float(majority_ratio),
        "num_cosponsors": safe_int(num_cosponsors),
        "num_committees": safe_int(num_committees),
        "num_actions": safe_int(num_actions),
        "days_since_intro": safe_int(days_since_intro),  # ✅ intro date–based
        "intro_year": safe_int(intro_year),
        "intro_month": safe_int(intro_month),
        "is_house_bill": safe_int(is_house_bill),
        "is_senate_bill": safe_int(is_senate_bill),
        "total_support": safe_int(total_support),
        "has_cosponsors": safe_int(has_cosponsors),
...
def predict_bill(
    ...
    try:
        ...
        feats = engineer_20_features_from_bill(bill)
